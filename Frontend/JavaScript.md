...
JavaScript 参数按值传递。对象传递的是引用值的副本：修改对象属性会被外部看到，给参数重新赋值则不会改变调用者的变量。

函数声明不限定实参数量，也不会自动检查参数类型；缺少的参数通常为 `undefined`。
...
- `obj.fn()`：通常为 `obj`。
- `new Fn()`：为正在创建的实例。
- 独立调用 `fn()`：严格模式下为 `undefined`；非严格模式下会替换为全局对象，在浏览器中通常是 `window`。
- DOM 事件的普通监听函数：通常为 `event.currentTarget`。
- `call`、`apply`、`bind`：显式指定普通函数的 `this`。

箭头函数没有自己的 `this`，使用外层作用域的值，不能通过上述方法重新绑定。

参考：[MDN this](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this)。下面独立调用返回 `window` 的示例以浏览器非严格模式为前提。
...
`call(null, ...)` 会显式传入 `null`：严格模式下保留 `null`，非严格模式下会替换为全局对象，不能把它理解成“保持原来的 this”。
