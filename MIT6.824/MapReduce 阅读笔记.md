...
**主节点处理**：主节点收到消息后，会检查该 `Map` 任务是否已经完成过（可能由于网络延迟等原因收到重复消息）。如果是已经完成的 `Map` 任务，主节点会忽略该消息；否则，主节点会将这 R 个文件的名称记录在其内部的数据结构中。这个数据结构用于跟踪和管理 `Map` 任务的输出位置信息，以便在 `Reduce` 任务需要时能够准确地获取数据。通过这种方式，主节点能够有效地协调 `Map` 任务和 `Reduce` 任务之间的数据传递，确保整个 `MapReduce` 计算过程的顺利进行，即使在存在故障和分布式处理的复杂环境下，也能保证计算结果的正确性和一致性，与无故障顺序执行的结果相同。
